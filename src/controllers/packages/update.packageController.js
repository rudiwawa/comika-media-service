...
const service = async function (req, res, next) {
  ...
  try {
    const requestDB = await Product.update(payload, { where: { id: body.id } });
...
const validation = [
  ...
  body("code").custom(async (value, { req }) => {
    return Product.findOne({ where: { slug: value, id: { [Op.ne]: req.body.id } } }).then((response) => {
  ...
  body("availableTo", "batas publish tidak boleh kosong")
    .notEmpty()
    .custom((value, { req }) => {
      console.log(moment(value).diff(moment(req.body.publishedAt)), "-------------–");
